...
public class SortItems_TASKB extends SequentialCommandGroup {
    ...
    public SortItems_TASKB(Map<String, Pose2d> pointMap) {
        super(
                new PickItemfromBin(),
                new MoveCamera(286),
                new SelectCommand(
                        Map.ofEntries(                                                                                                                                  // in SortItems these used to be T1, T2 and T3 but here its changed
                                Map.entry(CommandSelector.ONE, new SequentialCommandGroup(new MovetoB(()->RobotContainer.m_Grid.findGotoPos(RobotContainer.m_points.getPoint("RedTarget").getTranslation(), 0.6)), new MoveRobot(1, 0.05, 0, 0, 0.4))),
                                Map.entry(CommandSelector.TWO, new SequentialCommandGroup(new MovetoB(()->RobotContainer.m_Grid.findGotoPos(RobotContainer.m_points.getPoint("GreenTarget").getTranslation(), 0.6)), new MoveRobot(1, 0.05, 0, 0, 0.4))),
                                Map.entry(CommandSelector.THREE, new SequentialCommandGroup(new MovetoB(()->RobotContainer.m_Grid.findGotoPos(RobotContainer.m_points.getPoint("BlueTarget").getTranslation(), 0.6)), new MoveRobot(1, 0.05, 0, 0, 0.4)))),
                        SortItems_TASKB::selectTarget),
                // Lifts arm
                new DetectionPosition(),
                // sets cvMode to trolley alignment
