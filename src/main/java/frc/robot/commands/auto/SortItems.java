  ...
  public SortItems() 
    ...
    public SortItems(Map<String, Pose2d> pointMap) 
    {
        ...
        new SelectCommand(
            Map.ofEntries(
                Map.entry(CommandSelector.ONE,new MovetoB(Layout.Convert_mm_Pose2d(Layout.PickUpBinPos))),
                Map.entry(CommandSelector.TWO, new MovetoB(pointMap.get("bin"))) // Need to offset from found bin point
                ), 
            SortItems::selectBin
        ),
        new SelectCommand(
            Map.ofEntries(
                Map.entry(CommandSelector.ONE,new Rotate2Orientation(Layout.Convert_mm_Pose2d(Layout.PickUpBinPos).getRotation().getDegrees())),
                Map.entry(CommandSelector.TWO, new Rotate2Orientation(pointMap.get("bin").getRotation().getDegrees()))
                ), 
            SortItems::selectRotation
        ),
