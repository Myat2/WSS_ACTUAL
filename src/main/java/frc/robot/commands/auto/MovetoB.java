...
public class MovetoB extends SequentialCommandGroup {
    ...
    static private final CentripetalAccelerationConstraint m_CurveConstraint = new CentripetalAccelerationConstraint(
            0.5);
    ...
    public void initialize() {
        ...
        if (AstarPathWayPoints != null) {
            for (Node n : AstarPathWayPoints) {
                ...
                if (n instanceof Tile) {
                    ...
                    System.out.printf("x,y=%d,%d, end=%f,%f\n", t.getX(), t.getY(), Layout.Convert_cell_m(pt).getX(),
                            Layout.Convert_cell_m(pt).getY());
                }
            }
        } else {
        ...
        m_Trajectory =
                // myGenerateTrajectory.generateTrajectoryClampedCubic(m_pathWayPoints,
                // m_Config, 0.05);
