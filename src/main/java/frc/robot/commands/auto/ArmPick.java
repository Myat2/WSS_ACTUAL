...
public class ArmPick extends MoveArm {
    ...
    public void initialize() {
        // xgoal = 0.335 - (getItemY(Globals.curItem) - 120) * Globals.convertPxToMM +
        // 0.012;
        Translation2d pos;
        if (m_type == 0) { // MOves arm in the x direction
            if (Globals.curItemType == 0)// for cokeU
                ...
                ratio = 1;
            double x = m_arm.getArmPos().getX() + Globals.camera_offset
                    - (Globals.curItemY - m_vision.getResolution(1) / 2) * Globals.convertPxToM * ratio;
            ...
            double x = m_arm.getArmPos().getX() + ((400 - RobotContainer.m_vision.getLine()[1]) * TrolleyconvertPxToM)
                    - 0.05;
