...
hsvVal = np.loadtxt('hsvVal.txt',delimiter=',', dtype=int)
...
while True:
    ...
    _, contours, hie = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    for cnt in contours:
...
np.savetxt('hsvVal.txt', hsvVal, delimiter=',')
